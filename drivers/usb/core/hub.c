...
#if 0 /* org */
#else /* Willtek 240808: to remove chinese language */
	{
		int i;
		if(udev->product) {
			for(i = 0; ; i++) {
				if(udev->product[i] == 0) break;
				else if(udev->product[i] >= 0x80) {
					udev->product[i] = 0;
					break;
				}
			}
		...
		if(udev->manufacturer) {
			for(i = 0; ; i++) {
				if(udev->manufacturer[i] == 0) break;
				else if(udev->manufacturer[i] >= 0x80) {
					udev->manufacturer[i] = 0;
					break;
				}
			}
		}
